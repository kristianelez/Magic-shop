...
import type { Customer, Product } from "@workspace/db/schema";
...
const seasonalFactors: SeasonalFactors = {
  Osvježivači: { ljeto: 1.3, zima: 0.8, proljeće: 1.1, jesen: 1.0 },
  "Sredstva za čišćenje": { ljeto: 0.9, zima: 1.2, proljeće: 1.3, jesen: 1.0 },
  "Dezinfekciona sredstva": { ljeto: 0.9, zima: 1.4, proljeće: 1.0, jesen: 1.1 },
  "WC sredstva": { ljeto: 1.0, zima: 1.1, proljeće: 1.2, jesen: 1.0 },
  default: { ljeto: 1.0, zima: 1.0, proljeće: 1.0, jesen: 1.0 },
};
...
async function analyzeCustomerPurchasePattern(
  ...
  const totalOrderValue = sales.reduce(
...
function replaceSpecialProducts(products: string[], customerId: number): string[] {
  return products
    .filter(
      (product) => !product.toLowerCase().includes("14' scorch-brite standard range floor pad"),
    )
    .map((product) => {
      if (
        product.toLowerCase().includes("stochbrite") ||
        product.toLowerCase().includes("3m stochbrite")
      ) {
...
async function suggestProductsForCustomerOptimized(
  ...
  const customerTypeProducts = pattern.customerType
    ? allProducts.filter(
        (p) => p.recommendedFor && p.recommendedFor.includes(pattern.customerType!),
      )
    : allProducts;
  ...
  const seasonalProducts = customerTypeProducts
    .filter((p) => getSeasonalFactor(p.category) > 1.1)
  ...
  for (const category of essentialCategories) {
    if (!pattern.favoriteCategories.includes(category)) {
  ...
  if (pattern.daysSinceLastPurchase >= pattern.averageDaysBetweenPurchases) {
    ...
    reasons.push(`Zadnja kupovina je bila prije ${pattern.daysSinceLastPurchase} dana`);
  } else {
  ...
  if (notYetPurchasedFromEssentials.length > 0) {
    reasons.push(
      `Novi artikli za ponuditi: ${notYetPurchasedFromEssentials.slice(0, 2).join(", ")}`,
    );
  }
  ...
  return { products: suggestedProducts, reasoning: reasons.join(". ") + "." };
}
...
function buildFirstTimeRecommendations(
  ...
  selectedProducts.push(scotchBriteProducts[useAlternate ? 1 : 0]);

  if (selectedProducts.length < 3) {
    const remainingProducts = relevantProducts
      .filter((p) => !selectedProducts.includes(p.name))
      .slice(0, 6 - selectedProducts.length);
    for (const p of remainingProducts) selectedProducts.push(p.name);
  }
  ...
  return { products: selectedProducts, reasoning: reasons.join(". ") + "." };
}
...
export async function generateLocalRecommendations(): Promise<LocalRecommendation[]> {
  ...
  const allProducts = cachedAllProducts;
  ...
  for (const customer of allCustomers) {
    ...
    if (!pattern) {
      const { products: rawProducts, reasoning } = buildFirstTimeRecommendations(
        customer,
        allProducts,
      );
      const prods = replaceSpecialProducts(rawProducts, customer.id);
    ...
    const needsContact =
      pattern.daysSinceLastPurchase >= pattern.averageDaysBetweenPurchases * 0.8;
    if (!needsContact) continue;

    const { products: rawProducts, reasoning } = await suggestProductsForCustomerOptimized(
      pattern,
      allProducts,
    );
    const prods = replaceSpecialProducts(rawProducts, pattern.customerId);
    if (prods.length === 0) continue;
