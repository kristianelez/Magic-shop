...
- **`session` table is part of `shared/schema.ts`**: although `connect-pg-simple` creates the table on demand via `createTableIfMissing: true`, the table is also declared in the Drizzle schema (`session` pgTable with `sid` varchar PK, `sess` json, `expire` timestamp(6) + `IDX_session_expire` index). Without this, every `npm run db:push` in the post-merge script flagged the existing `session` table as drift and proposed a destructive DROP, which when accepted in past runs wiped all sessions and produced `relation "session" does not exist` on the next request. As an extra safety net, `server/index.ts` runs an idempotent `ensureSessionTable()` (CREATE TABLE IF NOT EXISTS + CREATE INDEX IF NOT EXISTS) inside the startup async IIFE before `server.listen()`, so the table is guaranteed to exist before the first HTTP request reaches the session middleware even if the DB was just reset.
